...
import { DiscogsMasterAlbumType } from "@/types/discogs";
...
const AlbumDetailsPage = async ({
  ...
  const album: DiscogsMasterAlbumType = await getAlbumFromDiscogsMasterId(
    params.masterId,
  );
  if (album.message === "Master Release not found.") {
    return <main className="flex-1 p-4">Album not found.</main>;
  }

  return (
    <main className="flex-1 p-4">
      <h1 className="w-full rounded bg-primary px-2 text-center text-3xl font-semibold uppercase tracking-tight text-white">
        Album Details
      </h1>
      <div className="b mt-4 flex flex-col justify-around md:flex-row">
        <div className="flex flex-col items-center gap-4 md:items-start">
          <h2 className="text-2xl">
            <span className="font-semibold">{album.title} </span>({album.year})
          </h2>
          ...
          <div className="flex gap-4">
            ...
            <Button variant="secondary" asChild>
              <a href={album.artists[0].resource_url} target="_blank">
                <DiscogsBrandLogo />
                Artist
              </a>
            </Button>
            {album.videos && (
              <Button variant="secondary" size="icon" asChild>
                <a href={album.videos[0].uri} target="_blank">
                  <Youtube />
                </a>
              </Button>
            )}
          </div>
          <div className="relative aspect-square w-[20rem] shadow">
            <Image
        ...
        <div className="flex flex-col items-center gap-4">
          <div className="">
            <h3 className="text-xl font-semibold">Tracklist</h3>
            {album.tracklist.map((track) => (
              <div key={track.position} className="flex items-center gap-4">
                <p>{track.position}</p>
                <p>{track.title}</p>
          ...
          <div className="relative aspect-video w-[24rem] shadow">
            <Image
              src={album.artists[0].thumbnail_url}
              alt=""
              fill
